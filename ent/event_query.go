...
// QueryRsvps chains the current query on the "rsvps" edge.
func (eq *EventQuery) QueryRsvps() *EventRSVPQuery {
	query := &EventRSVPQuery{config: eq.config}
...
// WithRsvps tells the query-builder to eager-load the nodes that are connected to
// the "rsvps" edge. The optional arguments are used to configure the query builder of the edge.
func (eq *EventQuery) WithRsvps(opts ...func(*EventRSVPQuery)) *EventQuery {
	query := &EventRSVPQuery{config: eq.config}
...
func (eq *EventQuery) sqlAll(ctx context.Context) ([]*Event, error) {
	...
	if query := eq.withRsvps; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Event)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Rsvps = []*EventRSVP{}
		}
		query.withFKs = true
		query.Where(predicate.EventRSVP(func(s *sql.Selector) {
			s.Where(sql.InValues(event.RsvpsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.event_rsvps
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "event_rsvps" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "event_rsvps" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Rsvps = append(node.Edges.Rsvps, n)
		}
	}
